...
mycursor.execute("CREATE TABLE list_items(list_id INT REFERENCES lists(list_id), fruit_id INT REFERENCES fruits(fruit_id), quantity INT);")

mycursor.execute("CREATE TABLE requests(to_id INT references users(user_id),from_id INT references users(user_id));")

mycursor.execute(
    "CREATE TABLE friends(friend_one INT REFERENCES users(user_id),friend_two INT REFERENCES users(user_id),permission ENUM('y', 'n') DEFAULT 'n');")
